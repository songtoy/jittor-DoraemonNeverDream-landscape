...
class SPADEGenerator(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        if (opt.num_upsampling_layers == 'most'):
            ...
            final_nc = (nf // 2)
        self.conv_img = nn.Conv(final_nc, 3, 3, padding=1)
    ...
    def execute(self, input, z=None):
        ...
        if True:
            if (z is None):
                z = jt.randn((input.shape[0], self.opt.z_dim), dtype="float32", requires_grad=False)
            x = self.fc(z)
...
class Pix2PixHDGenerator(BaseNetwork):
    ...
    def __init__(self, opt):
        super().__init__()
        input_nc = ((opt.label_nc + (1 if True else 0)) + (0 if True else 1))
        norm_layer = get_nonspade_norm_layer(opt, opt.norm_G)
        activation = nn.ReLU()
        model = []
        model += [nn.ReflectionPad2d((opt.resnet_initial_kernel_size // 2)), norm_layer(nn.Conv(input_nc, opt.ngf, opt.resnet_initial_kernel_size, padding=0, bias=False)), activation]
        mult = 1
        for i in range(opt.resnet_n_downsample):
            model += [norm_layer(nn.Conv((opt.ngf * mult), ((opt.ngf * mult) * 2), 3, stride=2, padding=1, bias=False)), activation]
            mult *= 2
        ...
        for i in range(opt.resnet_n_downsample):
            ...
            model += [norm_layer(nn.ConvTranspose(nc_in, nc_out, 3, stride=2, padding=1, output_padding=1, bias=False)), activation]
            mult = (mult // 2)
